...
/** `createComponent` returns the real `GridviewPanel` type; unwrap to the stub's shape. */
function apiOf(panel: { api: unknown }): FakeApi {
	return panel.api as FakeApi
}

describe('createGridviewFactory', () => {
	beforeEach(() => {
		vi.clearAllMocks()
	...
	it('releases state and unmounts on dispose', () => {
		...
		// `getComponent()` is protected on the real `GridviewPanel`; reach it
	...
	it('forwards the gridview context to the mount', () => {
		const registry = new GridviewWidgetRegistry()
		registry.register('a', { component: { name: 'Cell' } } as never)
